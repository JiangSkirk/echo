...
def test_run_local_gate_receipt_requires_matching_filename(tmp_path: Path) -> None:
    evidence_dir = tmp_path / "evidence"
    with pytest.raises(ValueError, match="receipt filename must be"):
        run_local_gate_receipt(
            gate_name="echo",
            argv=["/bin/echo", "gate-ok"],
            receipt_path=evidence_dir / "final" / "wrong-name.receipt.json",
            repo_root=tmp_path,
            evidence_dir=evidence_dir,
        )
...
def test_run_local_gate_receipt_source_drift_exits_nonzero(tmp_path: Path) -> None:
...
def test_run_local_gate_receipt_source_drift_main_exit_code(
    ...
    monkeypatch.setattr("scripts.run_local_gate_receipt.run_local_gate_receipt", _fake_run)
    monkeypatch.setattr(
        "sys.argv",
        [
            "run_local_gate_receipt.py",
            "--gate-name",
            "ruff",
            "--evidence-dir",
            str(evidence_dir),
            "--receipt",
            str(evidence_dir / "final" / "ruff.receipt.json"),
            "--",
            ".venv/bin/ruff",
            "check",
            "js/",
    ...
    from scripts.run_local_gate_receipt import main

    assert main() == 2


def test_valid_local_gate_receipt_rejects_true_impersonating_ruff(tmp_path: Path) -> None:
    _ensure_repo_toolchain(tmp_path)
    evidence_dir = tmp_path / "evidence"
    digest = release_source_digest(tmp_path)
    stdout = evidence_dir / "gates" / "ruff.stdout.txt"
    stderr = evidence_dir / "gates" / "ruff.stderr.txt"
    stdout_sha = _write_capture(stdout, "ok\n")
    stderr_sha = _write_capture(stderr, "")
...
def test_valid_local_gate_receipt_rejects_reversed_timestamps(tmp_path: Path) -> None:
    _ensure_repo_toolchain(tmp_path)
    evidence_dir = tmp_path / "evidence"
    digest = release_source_digest(tmp_path)
    stdout = evidence_dir / "gates" / "ruff.stdout.txt"
    ...
    payload["start_utc"] = "2026-07-23T06:00:02Z"
    payload["end_utc"] = "2026-07-23T06:00:01Z"
    assert (
        _valid_local_gate_receipt(
            payload,
            root=tmp_path,
...
def test_validate_final_local_gate_evidence_rejects_wrong_filename(tmp_path: Path) -> None:
    _ensure_repo_toolchain(tmp_path)
    evidence_dir = tmp_path / "evidence"
    ...
    (final_dir / "not-ruff.receipt.json").write_text(
        json.dumps(receipt, indent=2) + "\n",
        encoding="utf-8",
    )
    ...
    assert "ruff:filename_mismatch" in report.blockers


@pytest.mark.parametrize("missing_gate", ["slo_run_3", "soak_3600"])
def test_validate_final_local_gate_evidence_rejects_deleted_slo_or_soak(
    tmp_path: Path,
    missing_gate: str,
) -> None:
    evidence_dir = tmp_path / "evidence"
    ...
    (final_dir / f"{missing_gate}.receipt.json").unlink()
    report = validate_final_local_gate_evidence(
        tmp_path,
        final_dir=final_dir,
        evidence_dir=evidence_dir,
    ...
    assert report.all_local_gates_passed is False
    assert f"{missing_gate}:receipt_missing" in report.blockers


def test_validate_final_local_gate_evidence_requires_full_required_set(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    evidence_dir = tmp_path / "evidence"
