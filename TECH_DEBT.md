...
| WP9 | File Cell 只从 socket 收 package；staging、规范 diff、CAS/原子 rename、owner-root、symlink/hardlink/设备/NFC/casefold/挂载逃逸防护已落地 | WP7–WP9 回归 84 passed；全 Orin 399 passed | AppShell turn 到 Orin `task_id + DirectoryHandle` 尚无可信产品绑定，启用后工具链 fail-closed；Personal 精确 diff 审批 UI/证明未实现；真 APFS/断电耐久性未测 |
