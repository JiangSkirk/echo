...
def scan_plugin_source(plugin_id: str, source: str, filename: str = "<plugin>") -> PluginScanResult:
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Call):
            func = node.func
            if isinstance(func, ast.Attribute) and func.attr in {"write_text", "write_bytes", "mkdir"}:
                # Allow if it's using Path / plugin_dir (we can't statically verify the target)
                # So we just flag it for review
    ...
    # 3. Decide block/allow
    # Block on: syntax error, eval/exec/compile, os.system, subprocess, socket imports
    blocked = any(
        f.startswith(("disallowed_builtin:", "disallowed_import:", "syntax_error:"))
        for f in risk_flags
    )
