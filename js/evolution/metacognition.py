...
class MetacognitionLoop:
    ...
    def _analyze_evolution(self) -> list[dict[str, Any]]:
        ...
        for skill_id, total_success, total_executions, variant_count in rows:
            success_rate = float(total_success) / float(total_executions) if total_executions > 0 else 1.0
            stats.append({
                "skill_id": skill_id,
    ...
    def _auto_apply_proposals(self, proposals: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for proposal in proposals:
            area = proposal["area"]
            if area == "compression" and self.compression_feedback is not None and self.compression_config is not None:
                ...
                if recs.get("needs_adjustment"):
                    for param, data in recs.get("recommendations", {}).items():
                        if hasattr(self.compression_config, param):
                            ...
                            self.compression_feedback.apply_adjustment(param, new_value, data.get("reason", ""))
                            actions.append({
                                "area": area,
